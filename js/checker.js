...
function checkPicture(text) {
    if(!text)return null;
    let whiteList = new Array(".jpg",".png",".bmp",".jpeg");
    s=text.slice(text.lastIndexOf("."))
    for(let x=0;x<whiteList.length;x++){
        if(whiteList[x]===s){
            return null;
    ...
    return '文件不合法: 上传文件扩展名必须为.jpg/.jpeg/.png/.bmp';
}
...
function checkDDL(date) {
    var time1 = new Date();
    time1.setDate(time1.getDate()+14);
    var y2 = date.split("-")[0];
    var m2 = date.split("-")[1];
    var d2 = date.split("-")[2];
    var time2 = new Date(y2, m2 - 1, d2, 23, 59, 59);
    if (time1 - time2 > 0) return 'DDL必须设为下单时间之日起14天以后！';
    else return null;
}
...
function checkRegister() {
    ...
    if (registerform.QQ.value == "") {
        return checkError('QQ号不能为空！');
    } else {
        errMsg = checkQQ(registerform.QQ.value);
        if (errMsg) {
            return checkError('QQ号不合法: ' + errMsg);
        }
    }
...
function checkPushpot() {
    ...
    if (!document.getElementById("peopleNumber").selectedIndex) {
        return checkError('请选择所需人数！');
    }
    ...
    if (!productsChosen) {
        return checkError('您没有选择所需宣传品类型！');
    }
