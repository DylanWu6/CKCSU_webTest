                ...
                <table border=1 width="960px">
                    ...
                    <?php
                        ...
                        while($row = mysqli_fetch_array($result)){
                            ...
                            echo '<td>' . $usr . '</td>';
                            echo '<td>' . $row['name'] . '</td>';
                            $res = mysqli_query($conn, "SELECT * FROM checkin WHERE username = '$usr' AND DATE_FORMAT(lastdate, '%Y-%m-%d') = DATE_FORMAT('$curDate', '%Y-%m-%d')");
                            ...
                            if($usr == $myUsername){
                                echo '<td>是</td>';
                                if($row['pullAccess'] == 1) echo '<td>是</td>';
                                else echo '<td>否</td>';
                                ...
                                echo '<td>
                                        ...
                                        &emsp;&emsp; <button onclick="changePullAccess(\'' . $usr . '\')"> 更改接单权限 </button>
...
<script>
    function setadmin(username){
        ...
        if(r) {
            window.location.href = "useradmin_submit.php?username=" + username + "&type=setadmin";
        }
    }
    function deladmin(username){
        var r = confirm('您即将取消用户 ' + username + ' 的管理员权限\n是否确认？');
        if(r) {
            window.location.href = "useradmin_submit.php?username=" + username + "&type=deladmin";
        }
    }
    function resetPassword(username){
        var r = confirm('您即将重置用户 ' + username + ' 的密码为 abcd1234\n重置操作不可逆，是否确认？');
        if(r) {
            window.location.href = "useradmin_submit.php?username=" + username + "&type=reset";
        }
    }
    function delUser(username){
        var r = confirm('您即将删除用户 ' + username + '\n删除操作不可逆，是否确认？');
        if(r) {
            window.location.href = "useradmin_submit.php?username=" + username + "&type=delete";
        }
    }
    function changePullAccess(username){
        var r = confirm('您即将改变用户 ' + username + ' 的接单权限\n是否确认？');
        if(r){
            window.location.href = "useradmin_submit.php?username=" + username + "&type=changePullAccess";
        }
    }
    function closeRegister(){
        var r = confirm('您即将关闭注册通道\n是否确认？');
        if(r){
            window.location.href = "useradmin_submit.php?type=registerAccessClose";
        }
    }
    function openRegister(){
        var r = confirm('您即将开启注册通道\n是否确认？');
        if(r){
            window.location.href = "useradmin_submit.php?type=registerAccessOpen";
        }
    }
</script>
</body>
